...
scope = sorted([*set(raw_data["Country"])]); scope.append("Global emissions") # selectable countries + GLOBAL VIEW
...
def update_graph(selection):
    ...
    if selection == "Global emissions":
        ...
        fig = px.area(data, x="Year", y="CO2 emissions", color="Sector", template="none")
    else:
